...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Use a simple webhook to send email
      const webhookData = {
        to: 'jhamonty302@gmail.com',
        subject: `New Contact Form Submission from ${name}`,
        html: emailTemplate,
        from: 'Metaarth-Capital <noreply@metaarthcapital.com>',
        replyTo: email,
        name: name,
        email: email,
      ...
      console.log('To: jhamonty302@gmail.com');
      console.log('Subject: New Contact Form Submission from ' + name);
      console.log('From: Metaarth-Capital <noreply@metaarthcapital.com>');
      console.log('Reply-To: ' + email);
      console.log('HTML Content Length: ' + emailTemplate.length + ' characters');
      console.log('-----------------------------------');
      ...
      return NextResponse.json(
        { 
          message: 'Contact form received successfully. We will get back to you soon.',
          emailSent: false,
          note: 'Email service temporarily unavailable, but your message was received.'
        },
        { status: 200 }
      );
    }
