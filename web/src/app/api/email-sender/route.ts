...
export async function POST(request: NextRequest) {
  try {
    ...
    // In a real production environment, you would send this emailContent via an actual email service
    // For now, we'll simulate success and return the email content for manual sending
    console.log('Email Content Generated:', emailContent);

    return NextResponse.json(
      { 
        message: 'Contact form received successfully. We will get back to you soon.',
        emailContent: emailContent // Include the email content in the response
      },
      { status: 200 }
    );
